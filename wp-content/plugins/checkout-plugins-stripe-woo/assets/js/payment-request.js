		...
		if ( isProductPage && $( 'form.cart button.single_add_to_cart_button' ).length > 0 ) {
			...
			if ( 'above' === style.button_position ) {
				...
				if ( 'inline' === style.button_position ) {
					...
					const themetwentytwentyfiveButton = $( '.theme-Kadencebutton.single_add_to_cart_button' );

					if ( themetwentytwentyfiveButton.length > 0 ) {
						addToCartMinWidth = 100;
						addToCartMinWidthType = '%';
						themetwentytwentyfiveButton.css( makeWidth, addToCartMinWidth + addToCartMinWidthType );
						themetwentytwentyfiveButton.css( 'margin-top', '20px' );
